defmodule BetwiseWeb.SportsLive.Selections.FormComponent do
  ...
  def render(assigns) do
    ...
    <div>
      <.header>
        <%= @title %>
        <:subtitle>Add Selection record.</:subtitle>
      </.header>
      ...
      <.input
          ...
          type="select"
          label="Bet Type"
          options={Enum.map(@betTypes, fn type -> {type.bet_type_name, type.id} end)}
        />
  ...
  def update(%{selection: selection} = assigns, socket) do
    ...
    {:ok,
     socket
     |> assign(assigns)
     |> assign(:betTypes, BetTypes.list_bet_types())
